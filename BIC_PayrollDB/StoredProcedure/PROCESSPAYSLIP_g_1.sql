    ...
    BEGIN 
     ...
     ------------------------------------------------------------------------------------------
		...
		BEGIN TRY 
			...
			SELECT @pTotGrossIncome=ISNULL(SUM(ISNULL(M.AMOUNT,P.AMOUNT)),0)
			FROM  dbo.EMPLOYEEPAYMAPPING P 
			INNER JOIN dbo.PAYSLIPHEADS H ON P.HEADID=H.HEADID AND P.ISACTIVE=1 AND H.ISACTIVE=1 AND H.COMPID=@pCOMPID  AND H.TRANSACTIONTYPE=1
			LEFT JOIN dbo.PROCESSPAYSLIP M ON P.HEADID=M.HEADID AND M.EMPID=P.EMPID AND M.COMPID=@pCOMPID AND M.ISACTIVE=1 AND ISNULL(M.ISPROCESSED,0)=0 AND ISNULL(M.ISCLOSED,0)=0
			WHERE P.EMPID=@pEMPID

			SELECT @pTotDeduction=ISNULL(SUM(ISNULL(M.AMOUNT,P.AMOUNT)),0) 
			FROM  dbo.EMPLOYEEPAYMAPPING P 
			INNER JOIN dbo.PAYSLIPHEADS H ON P.HEADID=H.HEADID AND P.ISACTIVE=1 AND H.ISACTIVE=1 AND H.COMPID=@pCOMPID AND H.TRANSACTIONTYPE=2
			LEFT JOIN dbo.PROCESSPAYSLIP M ON P.HEADID=M.HEADID  AND M.EMPID=P.EMPID AND M.COMPID=@pCOMPID AND M.ISACTIVE=1 AND ISNULL(M.ISPROCESSED,0)=0 AND ISNULL(M.ISCLOSED,0)=0
			WHERE P.EMPID=@pEMPID
